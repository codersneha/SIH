...
export default function RetailerDashboard() {
  ...
  useEffect(() => {
    ...
    // Auto-refresh metrics and batches every 5 seconds
  ...
  const loadBatches = async () => {
    try {
      setLoading(true);
      const response = await dashboardService.getRetailerBatches();
  ...
  // Load accepted orders from localStorage (sales from order acceptance)
  const [acceptedOrders, setAcceptedOrders] = useState<any[]>([]);

  useEffect(() => {
    // Load accepted orders from localStorage
    const loadAcceptedOrders = () => {
      if (!user?.did) return; // Wait for user to load
      
      try {
        const ledgerEntries = JSON.parse(localStorage.getItem('unichain_economic_ledger_orders') || '[]');
        
        // Get orders accepted by this retailer (ORDER_ACCEPTED entries where this retailer is the acceptor)
        const retailerAcceptedOrders = ledgerEntries
          .filter((entry: any) => {
            return (
              entry.entryType === 'ORDER_ACCEPTED' &&
              entry.toDid === user.did &&
              entry.fromRole === 'CONSUMER'
            );
          })
          .map((entry: any) => ({
            batchId: `ORDER-${entry.orderId}`,
            productName: entry.product,
            quantity: entry.quantity || 0,
            unit: entry.unit || 'kg',
            sellingPrice: entry.quantity > 0 ? (entry.amount / entry.quantity) : 0,
            status: 'Sold',
            date: entry.timestamp,
            orderId: entry.orderId,
          }));

        setAcceptedOrders(retailerAcceptedOrders);
      } catch (error) {
    ...
    // Auto-refresh accepted orders every 5 seconds
    const interval = setInterval(() => {
      if (user?.did) {
        loadAcceptedOrders();
      }
  ...
  // Track total produce sold with arbitrary initial value
  const [totalProduceSold, setTotalProduceSold] = useState(() => {
    // Initialize with arbitrary value from localStorage or default
    const saved = localStorage.getItem('unichain_retailer_total_sold');
    return saved ? parseFloat(saved) : 1280; // Arbitrary initial value: 1280 kg
  });

  useEffect(() => {
    // Calculate total produce sold from accepted orders
    const calculateTotalSold = () => {
      try {
        // Get initial arbitrary value
        const initialValue = 1280;
        
        // Calculate from accepted orders
        const acceptedOrdersTotal = acceptedOrders.reduce(
          (sum, order) => sum + (order.quantity || 0), 
          0
        );
        
        // Calculate from batches sold
        const batchesSold = batches
          .filter(b => b.status === 'Sold')
          .reduce((sum, batch) => sum + batch.quantity, 0);
        
        // Total = initial arbitrary value + accepted orders + batches sold
        const total = initialValue + acceptedOrdersTotal + batchesSold;
        setTotalProduceSold(total);
        localStorage.setItem('unichain_retailer_total_sold', total.toString());
      } catch (error) {
    ...
    const interval = setInterval(calculateTotalSold, 5000);
    return () => clearInterval(interval);
  }, [batches, acceptedOrders]);

  // Calculate monthly KPIs
  const getMonthlyStats = () => {
    return {
      totalQuantity: totalProduceSold,
      unit: 'kg',
    };
  };
  ...
  // Combine batches and accepted orders for recent activity
  const allRecentActivity = [
    ...batches,
    ...acceptedOrders,
  ].sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
  
  const recentBatches = allRecentActivity.slice(0, 5);
  ...
  const getStatusBadge = (status: string) => {
    const statusMap: Record<string, { text: string; color: string; bgColor: string }> = {
      Received: { text: 'Received', color: 'text-blue-700', bgColor: 'bg-blue-50' },
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6">
        ...
        <div className="group relative bg-white rounded-2xl border-2 border-gray-200 shadow-lg hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-1 overflow-hidden">
          ...
          <div className="relative p-6">
            ...
            <p className="text-xs text-gray-400 mb-4">This Month (Including Accepted Orders)</p>
            <div className="flex items-baseline gap-2">
              <p className="text-4xl font-bold text-gray-900">
      ...
      <div className="bg-white rounded-2xl border-2 border-gray-200 shadow-lg overflow-hidden">
        ...
        {loading ? (
          ...
          <div className="text-center py-16">
            ...
            <p className="text-sm text-gray-500">Batches you receive or sell will appear here</p>
          </div>
        ) : (
